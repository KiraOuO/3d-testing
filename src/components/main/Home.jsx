...
import {ModelOne} from "../canvas/index.js";
...
const Home = () => {
    ...
    useEffect(() => {
        axios.get(`http://puppetpalm.com:8080/api/get-showcase-page?page=${currentPage-1}&size=${pageSize}`)
            .then(response => {
                setShoes(response.data.content);
            })
            .catch(error => {
    ...
    return (<section
        className={'relative mx-auto mb-0 pb-0 bg-mainbg ${styles.paddingX}'}>
        ...
        <div className="pb-12 flex justify-center items-center h-full mb-0 relative">
            ...
            <div
                className="mx-auto grid max-w-6xl gap-6 max-[375px]:grid-cols-1 grid-cols-2 lg:grid-cols-3">
                {shoes.map((shoe, index) => (<div
                    key={index}
            ...
            <div className="max-w-full dark:bg-[#2c2c2e] md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl bg-white p-6 rounded-lg mx-auto shadow-sm">
                <div className="flex justify-center">
                    <nav className="flex space-x-2" aria-label="Pagination">
                        {currentPage > 1 && (
                            <button onClick={handlePrevPage} className="relative dark:text-white inline-flex items-center px-4 py-2 text-sm border border-black hover:border-violet-100 text-black font-semibold cursor-pointer leading-5 rounded-md transition duration-150 ease-in-out focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10">
                                Previous
                            </button>
                        )}
                        <span className="relative dark:bg-[#2c2c2e] inline-flex items-center dark:hover:border-white dark:text-white px-4 py-2 text-sm font-medium text-black bg-white border border-black hover:bg-blue-300 cursor-pointer leading-5 rounded-md transition duration-150 ease-in-out focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10">
                                {currentPage}
                            </span>
                        {shoes.length === pageSize && (
                            <button onClick={handleNextPage} className="relative dark:text-white inline-flex items-center px-4 py-2 text-sm border border-black hover:border-violet-100 text-black font-semibold cursor-pointer leading-5 rounded-md transition duration-150 ease-in-out focus:outline-none focus:shadow-outline-blue focus:border-blue-300 focus:z-10">
                                Next
                            </button>
                        )}
                    </nav>
